####-----------------------------------------------------------####
#'
#'title: "Effective Coverage Estimate Cascade Model"
...
#'
#'Kenya introduced ACTs as the first line treatment policy and as such
#'we use more recent data from recent reviews for our adherence, cure rates and
#'resistance data assumptions for the surveys after 2004 onwards
#'
#'
...
hsb_path   <- "dhs_dir/data/kenya_2022_dhsrecode_hsb_hbhi.csv" #paste hsb file from the data folder for each year here in our example we are using 2022 file# #sets path to the excel health seeking behaviour mapping for each survey year
rx_path    <- "dhs_dir/data/kenya_2022_dhsrecode_rx_hbhi.csv" #paste rx file from the data folder for each year here in our example we are using 2022 file# #sets path to the excel health seeking behaviour mapping for each survey year
dhs_file   <- "dhs_dir/data/KEKR8BFL2022.DTA" #paste .dta file from the data folder for each year here in our example we are using 2022 file# #sets path to the DHS recode data file for each survey year
...
#'The above process is repeated for each survey year in the data folder for all the .dta files to produce output estimates
...
no_drug_data       <- function(dhs_file){
  ...
  #' Returns `True` iff no questions about drugs are available
  #' for a given survey.
  rx_mapping %>% 
    dplyr::filter(filename == dhs_file) %>%
    nrow(.) == 0
...
survey_metrics     <- function(df){
  ...
  # --- Bind admin1 and country level and calculate effective coverage ---
  metrics_df <- 
  ...
  # Add the name of the country and the name of the survey file
  metrics_df[["country"]] <-
    df %>% dplyr::select(country) %>% unique() %>% pull()
  metrics_df[["survey"]]  <-
...
effective_coverage <- function(access, formal, compl_f, compl_i){
  ...
  #' Assumptions on adherence and cure rates informed with literature 
  #'
  adh_act_f          <- 0.905 # Adherence to prescribing recommended country specific Antimalarial according to guidelines, according to the Kenya MALARIA HEALTH FACILITY ASSESSMENT 2023
  cure_act_f         <- 0.972 #(Therapeutic Efficacy of First line ACT based on the failure rates as reported on the Report on antimalarial drug efficacy, resistance and response, 10 years of surveillance (2010–2019))
  adh_act_i          <- adh_act_f * 0.8 # Assumes a lower adherence rate in the informal sector compared to the formal sector by a factor of 80%
  cure_act_i         <- cure_act_f - 0.083 #Assumes the cure rate reduces by the prevalence of sub optimal antimalarial in the country - Girma M et al, Quality of Antimalarial Drugs in East Africa: A Systematic Review
  cure_othrantimal_f <- 0.53 # Assumes the treatment failure rate for the other non country specific antimalarial used in Kenya as reported in Galactionova et al 2015
  cure_othrantimal_i <- cure_othrantimal_f - 0.33 #Assumes the cure rate reduces by the prevalence of sub optimal antimalarial in the country - Galactionova et al 2015
...
plot_waterfall <- function(waterfall_country, waterfall_year = NULL){
  ...
  waterfall(waterfall_df, 
    ...
    labs(title = paste("Effective coverage Cascade in", waterfall_country), 
...
# Calculate metrics for each survey of each country
...
# export to csv
output_path <- file.path(output_dir, "dhs_effcov_hbhi_allSrvy_v4_2022.csv")
